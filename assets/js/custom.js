(function ($) {
    $(document).ready(function() {
        ...
        $('.owl-offers-most-viewed-rtl').owlCarousel({
            loop: true,
            margin: 10,
            nav: true,
            dots: false,
        ...
        $('header .dropdown').click(function() {
            $(this).children('a').children('svg').toggleClass('tf-rotate-180');
        })

        $('header .navbar-mobile .dropdown a').blur(function() {
            $this = $(this);
            setTimeout(function() {
                $that = $this;
                $that.siblings('.dropdown-active').removeClass('dropdown-active')
                $that.children('svg').removeClass('tf-rotate-180');
            }, 400);
        })
        ...
        $.fn.jflatTimeline = function(options) {
            ...
            return this.each(function() {
                ...
                if (!selector.children(".dates-bar").length)
                    selector.children(".month-year-bar").after('<div class = "dates-bar"><a class = "prev"><i class = "fa fa-angle-left"></i></a><a class = "noevent">No available appointments</a><a class = "next"><i class = "fa fa-angle-right"></i></a></div>')
                ...
                for (i = 0; i < timelinedates.length; i++) {
                    ...
                    selector.children('.dates-bar').children('a.prev').after('<a data-index=' + i + ' data-date = ' + dateString + '><span class = "date">' + String(timelinedates[i].getDate()) + '</span><span class = "month">' + timelinedates[i].toString().split(' ')[0] + '</span></a>')
                }
